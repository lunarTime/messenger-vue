...
export function useMessageInput() {
    ...
    const canSendTypingStatus = computed(() => Boolean(chatStore.activeChatId && userStore.userId))
    const charactersRemaining = computed(() => VALIDATION_CONFIG.MESSAGE.MAX_LENGTH - message.value.length)
    const showCharacterCount = computed(() => message.value.length > VALIDATION_CONFIG.MESSAGE.MAX_LENGTH * 0)
    ...
    watch(
        ...
        (_, oldChatId) => {
            if (oldChatId && isTyping.value && userStore.userId) {
                setTypingStatus(oldChatId, userStore.userId, false).catch(console.error)
            }
    ...
    onUnmounted(() => {
        if (isTyping.value && canSendTypingStatus.value) {
            setTypingStatus(chatStore.activeChatId!, userStore.userId!, false).catch(console.error)
        }
    })
