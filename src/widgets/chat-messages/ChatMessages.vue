<script setup lang="ts">
    ...
    const { editMessage, deleteMessageForMe, deleteMessageForAll } = useMessageActions()
    ...
    const markAsRead = async (): Promise<void> => {
        if (!chatStore.activeChatId || !userStore.userId) return
    ...
    <div class="flex-1 min-h-0 relative">
        ...
        >
            <div>
                ...
                <p class="text-lg font-medium text-surface-700 dark:text-surface-300"> Нет сообщений </p>
                <p class="text-sm text-surface-500 dark:text-surface-400 mt-2"> Начните общение первым </p>
            </div>
        </div>
